...
**Invariant: a machine that has never held this strand's `Strand.Header` row must not commit to it.** The founder writes the Header in its bootstrap (`foundStrand`, or `addStrand` with `founder: true` / a row this machine published); every other machine can only receive it from a peer, so "no local Header" means "never synced".
...
- A **non-founder launch** brings its strand libp2p node, membership reconciler, revocation enforcer and peer-join backfill up as before, then reads `Strand.Header` once. If the row is held, the database is published at once — a restart or a hibernation resume over a store that has synced before is never gated, so offline-first writes on such a machine keep working. If it is not, the instance comes up **`'syncing'`** with `StrandInstance.database` unset, and re-reads the Header every `strandFirstSync.pollIntervalMs` (500 ms). Reads never invent a collection (optimystic's `Collection.open` resolves undefined on an authoritatively absent header and throws on an unreachable one), so the probe is safe to repeat; a throwing read is "not yet". The background loops read `instance.database` lazily, so they wait with it — the reconciler cannot seat a `Strand.Member` row into a forked table either.
- The moment the Header is readable, the database is published, the status flips to **`'active'`**, and `CadreNode` emits **`strand:writable`**. A founder request against a still-gated instance (`foundStrand` after a joiner-shaped attach) runs the bootstrap through the gate and opens it: writing the Header locally is the one legitimate way to become writable without a peer.

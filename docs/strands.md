...
- **A joiner's own write can still be lost after the gate opens — upstream.** Measured 2026-09-16 on the reference chat schema over direct connections, against the optimystic build at `6302f2e8`: a joiner that writes its participant and a message the instant `addStrand` resolves converged in 1 run of 5; the other 4 failed the message's foreign-key check because the joiner's just-committed participant row was not readable by its next statement. Optimystic is tracking that as its own read-your-own-write defect (filed from this measurement); `tickets/blocked/device-shape-join-write-blocked-on-optimystic-read-after-write.md` holds the sereus side. The gate's contribution is that the failure is now a loud constraint error, not a silent fork.

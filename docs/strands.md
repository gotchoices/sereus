...
- **`CadreNode.addStrand` resolves only once the strand is writable**, bounded by `strandFirstSync.timeoutMs` — **120 s**, sized for a machine joining through a relay on a slow link. Measured 2026-09-26 over a relayed path with a round trip of about 1.8 s, a machine holding nothing of the strand became writable 23-41 s after `addStrand` at optimystic's 1000 ms cohort read deadline, and 35-46 s at a 5000 ms one; the earlier 30 s default sat inside the first of those bands, so about half of those joins were rejected while their sync was progressing normally and went on to complete. `DEFAULT_STRAND_FIRST_SYNC_TIMEOUT_MS`'s doc comment carries the full measurement and what the larger budget costs. On timeout it rejects with `StrandAwaitingFirstSyncError` ("no member of this strand has been reachable since this machine joined") and leaves the launch up, still probing; the same call made again later completes the attach, as does awaiting `CadreNode.whenStrandWritable(strandId)` or the event. `addStrand({ awaitFirstSync: false })` returns the `'syncing'` instance immediately for a caller that wires its own peers first (the integration harness). There is deliberately no way to disable the gate.

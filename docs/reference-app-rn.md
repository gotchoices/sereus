...
**The first-sync wait is widened for the same reason, also for every node.** A phone that has just redeemed an invitation holds none of the strand's data, so cadre-core withholds the strand database until that data arrives from another member and rejects `addStrand` with the retryable `StrandAwaitingFirstSyncError` if it has not arrived within `strandFirstSync.timeoutMs` (see [`strands.md` → Joining](strands.md#joining-no-writes-before-the-first-sync)). Through a relay on a slow link that first sync is not quick: measured over a path with a round trip of about 1.8 s, a machine holding nothing of the strand became writable 23-46 s after `addStrand`, the spread depending on the cohort read deadline in force. cadre-core therefore defaults the wait to **120 seconds** (`DEFAULT_STRAND_FIRST_SYNC_TIMEOUT_MS`). This app sets nothing for it. The chat screens re-render on `strand:writable` ([`use-cadre.ts`](../packages/reference-app-rn/src/use-cadre.ts)), so a sync that lands after any budget still opens the screen — but [`joinClosedChatStrand`](../packages/reference-app-rn/src/chat-strand.ts) writes the joiner's app-level role right after `addStrand` resolves, so a join that times out has to be retried before that role exists.

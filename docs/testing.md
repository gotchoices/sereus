...
- **Async generator `finally` blocks in code that runs on React Native** (human-review-only; tripwire): Metro compiles async generators with Babel, and when a consumer stops early (`break` or `return` inside `for await`) the compiled `finally` silently skips everything after an `await` that is not its last action. So in an async generator a `finally` may `await` only as its final step; put multi-step async cleanup in a regular async helper and make its call that final `await`. Quereus hit this as a database lock that was never released (its tickets `async-generator-finally-await-leaks-under-babel` and, for a lint rule, `lint-async-generator-finally-tail-await`). Sereus's shipping code has one async generator today (`scanMemberPeers` in `cadre-core/src/strand-membership-writer.ts`, with no `try/finally`), so that rule is not copied here. If Sereus code that runs on React Native gains an async generator with a `try/finally`, adopt Quereus's rule in `eslint.config.mjs`.

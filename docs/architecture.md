...
- **Two yardsticks, not one — and only one of them is derived from the party.** Optimystic's cluster policy carries two size declarations with different jobs, and Cadre answers them differently.
  ...
  **When it takes effect.** Optimystic freezes the value when the libp2p node is built and offers no runtime setter, so the number is applied *by rebuild*. For a strand that is every wake from hibernation and every check-in cycle — many times a day — because `resumeStrand` re-resolves it beside the cohort seed and writes it back into the retained launch config. For the control network it is **once per launch**, because the control node is built before the database holding the membership rows exists — see the next point.

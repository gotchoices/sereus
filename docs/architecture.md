...
- **`addMemberByManager(db, { managerKeyPair, memberKey })`** — the sibling direct-admit branch of `Member.Authorized`: an manager signs `digest(new.Key)` and seats a member with no invite involved (for a party already trusted out-of-band).
...
- **`addManager(db, { byManagerKeyPair, newManagerKey })`** — an existing manager promotes a member to `Manager`. The existing-manager branch of `Manager.Authorized` verifies `digest(new.MemberKey = newManagerKey)` against an `Manager` row matching `context.ManagerKey`, so the signer signs the new key directly and binds itself as `context.ManagerKey`. Once the founder manager exists the `count(Manager) <= 1` bootstrap shortcut no longer applies (at commit the new row makes the count ≥ 2), so this genuinely exercises signature verification — a non-manager signer or a wrong-key signature is rejected. (The `Manager` table has **no** `MemberExists` constraint, so an manager key need not also be a `Member` row.)
- **`removeManager(db, { byManagerKeyPair, targetManagerKey })`** — delete an `Manager` row, either an **admin** removing a different manager (existing-manager branch) or an manager **resigning itself** (former-manager self branch). On a delete `coalesce(new.MemberKey, old.MemberKey)` binds `old.MemberKey = targetManagerKey`, so the signed payload is the target key for **both** branches and one context construction serves both — the caller selects the case purely by which keypair it passes (a different manager vs. the target's own).

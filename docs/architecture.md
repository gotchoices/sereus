...
**How the writers transact.** Every writer that changes membership rows (`consumeInvite`, `burnInvite`, `addMemberByManager`, `revokeMember`, `leaveStrand`, `registerMemberPeer`, `removeMemberPeer`, `addManager`, `admitManager`, `removeManager`, `sealStrand`) does its reads and signing first, then sends its whole transaction to Quereus as one `begin transaction; …; commit;` `exec` batch. Quereus runs a batch without letting another caller's statement in, so an app write issued meanwhile is not swept into the membership transaction. Each takes an optional trailing `StrandWriteOptions`. By default (`joinOpenTransaction: true`) a writer called while a transaction is open runs inside it, and the transaction's owner commits or rolls back, which is how specs compose a resign + revoke pair. Background callers that share the database with the app (the membership reconciler, the unpublish binding cleanup) pass `false`, and the writer then throws `StrandTransactionBusyError`, having written nothing. One gap stays open until Quereus can roll back a failed batch before releasing it (`../quereus/tickets/plan/exec-batch-as-one-transaction.md`): a statement that fails before `commit`, such as a primary-key collision, leaves the batch's transaction open for a moment, and an app write already queued behind it is lost with its rollback. The `NOTE:` on `execStrandTransaction` has the details.

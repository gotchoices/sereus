...
The `provider` field accepts either an `IRawStorage` instance (shared across all strands) or a factory function `(strandId: string) => IRawStorage` (recommended—creates isolated storage per strand). The factory pattern ensures each strand's data is partitioned, simplifying cleanup and preventing cross-strand interference. **Use the factory form for any persistent backend.** One instance shared across scopes also shares ONE cache wrapper (the helper below is memoized per inner instance) which no scope solely owns, so the first scope to stop retires the wrapper the still-running scopes are reading through — see `tickets/backlog/debt-storage-cache-wrap-unrefcounted.md`. An in-memory store is unaffected (it is never wrapped).

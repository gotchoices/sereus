...
> **Convergence prerequisites and current status.** The replicated `CadrePeer` form described above is the *design target*; it is **not yet the running behaviour**. Two things must hold for control-DB convergence, and the first is currently missing:
>
> - **The control tables must be network-backed.** `ControlDatabase.initialize()` registers the Optimystic plugin with `default_transactor: 'network'`, but it does **not** make `optimystic` the default vtab (no `setDefaultVtabName('optimystic')` / `setDefaultVtabArgs(...)`). So the `declare schema CadreControl` tables — which carry no per-table `using optimystic(...)` — today fall back to Quereus's built-in **in-memory** vtab: a control write is visible only on the writing node and never replicates. (Strand tables converge precisely because `connectToStrand` *does* set the default vtab + args + hydrate before applying its schema.) Wiring the control DB the same way is tracked in `control-db-network-backed`; it must also reconcile the deferred-`CHECK` / `committed.*` snapshot semantics the consent path depends on (`redeemInvitation`, `recordFormationUsage`, `FormationUsage.Monotonic`), which the network transactor does not yet honour the way the in-memory vtab does.
> - **The cohort/cluster must form.** Even once network-backed, convergence requires the responsible Optimystic cohort to form (cadre nodes actually connected, FRET routing populated) — not merely a bootstrap link. A write made while a node is alone (cohort ≤1) commits **local-only** and propagates only once the cohort forms (re-publish or pull-on-read repair). Production auto-connect for the control key is tracked separately in `control-network-cohort-discovery` — a distinct concern from network-backing (in-memory tables cannot replicate no matter how good connectivity is).
>
...
- **`addMemberByAuthority(db, { authorityKeyPair, memberKey })`** — the sibling direct-admit branch of `Member.Authorized`: an authority signs `digest(new.Key)` and seats a member with no invite involved (for a party already trusted out-of-band).
...
**Single-use layering.** The per-strand member join is single-use via `ConsumedInvite`'s primary key (`InviteKey`) — a distinct layer from the control network's `FormationUsage` single-use, which gates strand *formation* (cadre-operator consent), not member join. ⚠️ **Known platform gap:** the optimystic bootstrap-mode vtab transactor does not yet enforce primary-key uniqueness on `INSERT` (a duplicate-PK insert silently overwrites), so this single-use guarantee is **not currently enforced** and an invite can be replayed; tracked in `optimystic-insert-pk-uniqueness-not-enforced` (backlog), with a regression-sentinel test pinning the current behavior.
...
#### MemberPeer registration + authority rotation
...
- **`registerMemberPeer(db, { memberKeyPair, peerId })`** — a member records which network node (`PeerId`) acts on its behalf. The member **self-signs** `MemberKey || '|' || PeerId` with its own key; `MemberPeer.Authorized` verifies that signature against `coalesce(new.MemberKey, old.MemberKey)` — i.e. the member key itself — so a peer can only be registered by the very member it belongs to (no authority is involved). A deferred `MemberExists` additionally requires the `Member` row to already exist, so a peer for a non-member is rejected at commit. The write is **insert-if-absent** on the composite PK `(MemberKey, PeerId)` so a re-register on restart is a no-op (it never depends on the platform's unenforced PK-uniqueness — see the gap below); a member may register multiple **distinct** `PeerId`s (multi-device), each its own row. Peer **deletion** is out of scope: the schema's `MemberExists` reads `new.MemberKey` (null on delete), so a `MemberPeer` delete would need a schema tweak first.
- **`addAuthority(db, { byAuthorityKeyPair, newAuthorityKey })`** — an existing authority promotes a member to `Authority`. The existing-authority branch of `Authority.Authorized` verifies `digest(new.MemberKey = newAuthorityKey)` against an `Authority` row matching `context.AuthorityKey`, so the signer signs the new key directly and binds itself as `context.AuthorityKey`. Once the founder authority exists the `count(Authority) <= 1` bootstrap shortcut no longer applies (at commit the new row makes the count ≥ 2), so this genuinely exercises signature verification — a non-authority signer or a wrong-key signature is rejected. (The `Authority` table has **no** `MemberExists` constraint, so an authority key need not also be a `Member` row.)
- **`removeAuthority(db, { byAuthorityKeyPair, targetAuthorityKey })`** — delete an `Authority` row, either an **admin** removing a different authority (existing-authority branch) or an authority **resigning itself** (former-authority self branch). On a delete `coalesce(new.MemberKey, old.MemberKey)` binds `old.MemberKey = targetAuthorityKey`, so the signed payload is the target key for **both** branches and one context construction serves both — the caller selects the case purely by which keypair it passes (a different authority vs. the target's own).

**Authority-removal hazards.** ⚠️ **Known platform gap:** the optimystic bootstrap-mode transactor evaluates deferred (subquery-bearing) `CHECK` constraints only on `INSERT`, **not on `DELETE`** — `Authority.Authorized` is deferred, so the platform currently accepts **any** `Authority` delete regardless of signature, leaving `removeAuthority` unauthenticated at runtime (tracked in `optimystic-deferred-check-not-enforced-on-delete`, backlog, with a KNOWN GAP sentinel test pinning the behavior; the writer still emits a correctly-signed delete so it works unchanged once enforcement lands). ⚠️ **Schema hazard (independent of the gap):** `Authority.Authorized`'s `count(Authority) <= 1` bootstrap branch is true at commit whenever a delete drops the count to ≤ 1, so removing the strand's last (or second-to-last) authority is accepted regardless of signature — and removing the **last** one orphans the strand (no one can ever add another). A "min-one-authority" invariant is deferred to a future schema change rather than grown into the writer.

#### End-to-end coverage

Beyond the per-writer component specs (`cadre-core/test/strand-*`), the closed-strand membership lifecycle is exercised end-to-end across **two real `CadreNode`s** over libp2p in `integration-tests/.../strand-membership-closed-strand-e2e.integration.ts`. Both nodes attach the same directly-constructed closed `StrandRow` (`Type='c'`, `MemberPrivateKey` minted via `generateStrandMemberKey`) in `networked` mode with a manual strand-level dial; the founder (`founder:true`) bootstraps `Header`/`Member`/`Authority` while the joiner (`founder:false`) writes nothing on bring-up. The scenario then drives — and asserts accept **and** reject for — invite issue/consume, `registerMemberPeer`, `addAuthority`, and a final `App.Items` signed write by the newly-admitted member (tying the layer-2 `Strand.Member` admission to layer-3 sApp RBAC: the same key that joined the strand signs the sApp write). The writer-driven accept/reject cases run against the **founder's** strand DB (the authoritative DB where bootstrap seated the constraint-backing rows) and are the gating deliverable; cross-node replication of the founder's `Strand.*` bootstrap rows to the joiner is observed best-effort (and, in practice, observed reliably). Two networked-transactor quirks surfaced and are worked around in-test rather than asserted: a full **composite-PK point lookup** (`MemberPeer where MemberKey = ? and PeerId = ?`) is not reliably served — read the singleton row directly instead — and rejected writes assert only `throws` (no post-state rollback), per `optimystic-insert-pk-uniqueness-not-enforced` / the deferred-constraint-rollback gap.

...
The cache's soundness rests on **one live cache per backend, and one process per store** (upstream `docs/storage.md`, Invariants 1 and 5). The helper is memoized per storage instance so the overlapping seams cannot stack two caches over one store, and the wrap is idempotent so re-wrapping is safe. Two obligations follow for anyone writing a provider or a new call site: never hand a node or transactor a raw `IRawStorage` without routing it through the helper, and never point two stores at one path or keyspace. **A provider should return the same instance for a given key** — cadre-core calls it again whenever a strand runtime is rebuilt (hibernation wake), and most shipped providers do not yet honour that; `tickets/fix/strand-runtime-rebuild-remints-raw-storage.md` carries the fix.

...
- **Strand Solicitation API**: `createOpenInvitation()`, `formStrand()` with full `strand-proto` SessionManager integration via `StrandFormationManager`; outside approval of a redemption (an invite's `ValidationUrl`) lives in the formation-approval client (`createHttpFormationApprover()`), not in the solicitation service — see [`docs/api.md`](api.md)

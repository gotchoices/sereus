...
This closes `bug-strand-manager-authority-antireplay`: an approval is bound to one table, one action, and one row incarnation, and cannot be replayed once that incarnation is gone. Two consequences worth knowing: filing a tombstone is a *member* action — `Revocation.Authorized` verifies a delete's tombstone against a committed `Member` row — so revoking, clearing a peer binding, or resigning all require the acting manager to also be a member; `Manager.MemberExists` guarantees exactly that on every insert, so no manager is ever left holding those powers unable to use them. And `Revocation` is append-only and never pruned, so it grows with membership churn.

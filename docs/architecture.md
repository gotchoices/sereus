...
| `FormationUsage` | Append-only audit log of formation invite consumption: one row per redemption, recording which peer joined (`PeerId`, writer-asserted) and a per-redemption single-use nonce (`UsageStampId`). Both are bound into the approver sign-off digest that `Authorized` verifies for a `ValidationUrl` invite, which is what makes one approval spendable exactly once |

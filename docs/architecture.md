...
- **Entry point**: callers found a strand with `CadreNode.foundStrand(FoundStrandConfig)`, which publishes the `Strand` row and attaches as founder in one call and is safe to re-run. Founding is two control writes (`publishStrand` then `addStrand(…, founder: true)`), and hand-rolling the pair is not resumable: an app killed between them used to fail every later attempt on `UNIQUE constraint failed: Strand.Id`, or — if it resumed by attaching as a joiner — run the strand with an empty `Strand.Header`. `publishStrand` is therefore idempotent for identical `(Type, MemberPrivateKey)` content and throws a mismatch-naming error otherwise (never the raw uniqueness error, and never an overwrite; a concurrent founder that wins the insert race is resolved the same way, by re-read). `foundStrand` adopts an already-published row's STORED `MemberPrivateKey`, so a caller that mints a key per attempt must read the resolved key back off the returned row. `addStrand` alone remains the join/attach path.
  - **Known gap — founding is only guaranteed when `foundStrand` launches the instance.** `launchStrand` returns an already-tracked instance and silently drops the `founder` flag it was asked for, so whoever launches the strand FIRST decides whether the bootstrap runs. Two things can win that race: an app that attached the strand itself (the reference RN app's `strand:discovered` handler re-registers the sApp config and attaches after a restart, since nothing in the `Strand` row records who published it), or this node's own `StrandWatcher` — `foundStrand` publishes the row before calling `addStrand`, and `launchStrand` awaits `resolveCohortSeed` (a network round) between its tracked-instance check and `startStrand`, so a poll landing in that window launches the row as a joiner. Either way the strand comes up `active` with an empty `Strand.Header` and `foundStrand` reports success. Founder-ness is neither persisted locally nor observable on a tracked instance, so no caller can detect it; closing it is a representation change, tracked in `tickets/backlog/`.

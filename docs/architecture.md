...
**A provider is called once per scope per runtime, and cadre-core owns what it returns.** The scopes are `'control'` (once per `CadreNode.start()`) and each strand id (once per `startStrand`); a `CadreNode` holds the control store in a field, and `StrandInstanceManager` holds each strand's store keyed by strand id. A hibernation quiesce → resume rebuilds the libp2p node over that same store rather than re-entering the provider, so the wake lands on a warm cache — and, on an in-memory backend, on the strand's blocks rather than an empty store. The provider *is* called again for a scope whose runtime has stopped (`stopStrand`, or a `stop()` then `start()` cycle), so it must be able to return a store over the same durable backend a second time; it need not memoize to do so. When a scope's runtime stops, cadre-core calls `disposeStorageCache` on its wrapper — releasing that scope's claim, and with the last claim the shared pool registration a long-lived process would otherwise accumulate one of per stop — and never closes the underlying handle the embedder returned.

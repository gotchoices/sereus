...
> **Convergence prerequisites and current status.** The replicated `CadrePeer` form described above replicates in the wiring, and a party's control nodes now auto-connect; one durability gap for *production* convergence remains (write-while-alone):
>
...
> - **Write-while-alone durability (open).** A write made while a node is alone (its block's cluster ≤1) still commits **local-only** and propagates only once the cohort forms (re-publish or pull-on-read repair). Auto-connect shrinks this window but does not close it: an authority that writes in the instant before any sibling connects has a non-durable row. Remedying it (re-issue authority-authored writes on cohort growth) is tracked in `control-write-ensure-replicated`.
>

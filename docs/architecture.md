...
- **Plumbing**: `founder?: boolean` flows `CadreNode.addStrand(StrandConfig)` → `launchStrand` (which derives it from `FounderOwnerKey` when unset — an explicit flag wins) → `StrandInstanceManager.startStrand` → `buildStrandRuntime` → `StrandDatabase`. The bootstrap runs in `StrandDatabase.initialize()` *after* `connectToStrand` returns (schema is applied by then), gated on `founder === true`. A throw during bootstrap propagates out of `initialize()` so `buildStrandRuntime`'s rollback tears the half-built strand down.

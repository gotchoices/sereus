...
The node is given its identity via the child config's `identity.protobufKeyFile` (the installer's protobuf `identity.key`) or the `--identity-protobuf <path>` flag. Because that path is also where `cadre-cli` puts the node-local trusted-owner anchor (`trusted-owners.<partyId>.json`, see [architecture → Seed Delivery Protocol](architecture.md#seed-delivery-protocol)) and the node-local bootstrap-peer store (`bootstrap-peers.<partyId>.json`), `<dataDir>` holds the owner node's identity *and* its out-of-band trust *and* its retained cold-start dial targets — back up and restore them together.

**Donated nodes get the same treatment, scoped to their own workdir.** Every node the orchestrator spawns for a requester is written its own `identity.key` in `<rootDir>/<containerId>/` (generated on first spawn, reused on every later one) and launched with `--identity-protobuf` pointing at it. Two reasons, both load-bearing: the requester's cadre approved a specific peer id, so a per-process keypair would make the node a stranger to it after any restart; and the two node-local stores above open only when that path is configured, so without it a donated node lost the dial addresses its seed nominated on every restart. Its identity and both stores therefore live inside its workdir — and are destroyed with it when the loan is terminated (`removeContainer` deletes the workdir), which is the containment property the donation flow wants.

...
import { waitUntil, waitForCadrePeerConverged, controlNodeConfig, makeOwnOwner, randomPeerId } from '../harness/index.js';
...
describe('Control-cohort auto-convergence (no manual dial)', () => {
	it('B converges on an owner-written CadrePeer row via in-node reconcile (production cold-start only)', async () => {
		...
		try {
			...
			B = new CadreNode(controlNodeConfig({ partyId, privateKey: bKey, profile: 'transaction', reconcileMs: 2_000 }));
			await B.start();

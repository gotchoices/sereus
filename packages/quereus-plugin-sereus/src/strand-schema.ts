 ...
 * be mirrored there and vice versa. `test/strand-schema-drift.spec.ts` enforces it
 * — but only from THIS package, so an edit made in `schemas/strand.qsql` alone
 * stays green everywhere its author is likely to look; run that spec after either.
 *
...
export const STRAND_SCHEMA = `    table Header (
    ...
    table ConsumedInvite (
        ...
        -- A SEALED strand (a closed strand whose Manager table its sole manager has
        -- deliberately emptied — see the Manager table comment) admits nobody, and that
        -- includes the holder of an unspent invitation issued BEFORE the seal.
        -- cancelInvite itself needs a manager, so after sealing nobody could kill such
        -- an invitation — this gate is what kills it. The subquery reads the LIVE
        -- Manager table and defers to commit, so it sees the post-image: at least one
        -- manager must survive the transaction, and sealing plus consuming in the SAME
        -- transaction is rejected too. Blocking this insert blocks the whole join, for
        -- exactly the reason NotCancelled gives above.
        -- The Manager rows this reads are the ones THIS NODE can see, so the gate binds
        -- only once the seal has reached here — on a node still behind on the seal it
        -- passes, and a stranger holding a pre-seal invitation joins (the Manager table's
        -- seal-propagation NOTE).
    ...
    -- the NotRevoked / MinOneMember local-visibility notes and the Manager table's
    -- seal-propagation note; not solved here.
    ...
    table Manager (
        ...
        -- as the MinOneMember / NotRevoked / seal-propagation notes; not solved here.
        ...
        constraint NoUpdate check on update (false),
        -- A closed strand loses its LAST manager only by SEALING — a distinct,
        -- self-signed act (the 'seal' branch of Authorized below), never an ordinary
        -- resignation and never an admin removal. There is deliberately NO
        -- min-one-manager floor: a strand with no managers is frozen, not bricked.
        -- Every admission path (Invite, addMemberByManager, addManager) requires a
        -- Manager row, so a sealed strand's membership can never grow again — and that
        -- freeze is the privacy guarantee its remaining members are buying: nobody
        -- holds the power to admit a party who would then read the strand's whole
        -- history. Sealing is irreversible: the founding branch of Authorized closes
        -- for good once any manager stamp has been retired into Revocation.
        -- NOTE: a seal must PROPAGATE before it binds a given node — a node that has
        -- not yet converged on the delete still shows the manager row, so until the
        -- delete arrives that node admits as if unsealed. It is NOT only the
        -- ex-manager's own key that gains: every gate here reads the LOCALLY visible
        -- Manager rows, and ConsumedInvite.NotSealed is one of them, so a STRANGER
        -- holding an invitation issued before the seal can redeem it on such a node.
        -- Same convergence class as the MinOneMember / NotRevoked local-visibility
        -- notes; not solved here. Arrival IS covered, on the converged side:
        -- integration-tests' strand-membership-closed-strand-e2e gates a second node on
        -- the seal and then requires it to refuse every admission path.
        constraint Authorized check on insert, delete (
            ...
            -- The Revocation gate closes this branch FOREVER once any manager seat has
            -- been retired: founding happens exactly once per strand (Header, then
            -- Member, then Manager, with no Revocation rows in existence), so on a
            -- SEALED strand a lone surviving member cannot re-seat itself as a
            -- generation-0 founder and start admitting. Accepted, deliberate: between
            -- its own founding Member and Manager inserts a founder could, with its own
            -- key, file a junk Revocation('Manager', ...) tombstone (RowIsGone permits
            -- stamps naming no real row) and permanently block its own founding — pure
            -- self-harm needing the founder's private key; no other party can reach
            ...
            (old.MemberKey is null
                ...
                -- or removed by the departing manager ITSELF, over one of two
                -- self-signed, action-tagged digests bound to its own row's stamp. The
                -- POST-image manager count is what distinguishes the two: 'resign' is
                -- an ordinary departure that leaves at least one manager standing,
                -- while 'seal' is the delete that empties the table and permanently
                -- freezes admission (see the table comment above). The tag is what
                -- makes each a signed statement of THAT intent — a captured
                -- resignation approval can never seal, nor a seal approval merely
                -- resign — and the stamp is still what makes each single-use: once
                -- this manager is re-promoted under a fresh stamp, neither captured
                -- approval removes the new row. Both counts are subqueries, so the
                -- whole check defers to commit and the counts are the post-image.
                or (
                ...
                or (
                    old.MemberKey is not null
                        and old.MemberKey = context.ManagerKey
                        and (select count(1) from Manager) = 0
                        and verify(digest('Strand.Manager', 'seal', old.MemberKey, old.StampId), context.Signature, old.MemberKey, 'ed25519')
                )
    ...
    -- convergence class as the MinOneMember / Manager seal-propagation notes.

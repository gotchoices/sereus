...
declare schema CadreControl {
    ...
    table FormationUsage (
        Token text,
        UseNumber int,
        UsageStampId text not null unique,  -- single-use nonce for THIS redemption. The JOINING peer mints it and
                                        -- sends it in its contact message, having signed it into its own 'consent'
                                        -- digest (PeerConsented below); the responder inserts that same value and
                                        -- asks the approver about it, so it is bound into the 'vouch' digest below
                                        -- too and one sign-off is spendable exactly once. Neither side can swap it:
                                        -- a substituted nonce fails the joiner's consent signature.

    ...
    table OwnerKey (
        ...
        constraint Authorized check (
            ...
            (select count(1) from OwnerKey) <= 1
                ...
                -- Old owner authorizes by signing over THIS row (Key, StampId); single-use via unique StampId
                or (old.Key is not null and old.Key = context.OwnerKey and verify(digest(new.Key, new.StampId), context.Signature, old.Key, 'ed25519'))

                -- or other owners authorize by signing over THIS row (Key, StampId); single-use via unique StampId
                or exists (select 1 from OwnerKey A where A.Key = context.OwnerKey and verify(digest(new.Key, new.StampId), context.Signature, A.Key, 'ed25519'))
        )

...
declare schema CadreControl {
    ...
    table OwnerKey (
        ...
        constraint Authorized check on insert, delete (
            ...
            -- Bootstrap: first owner key needs no existing authorization. Gated on the
            -- PRE-transaction count, so a same-transaction swap of the sole owner (delete
            -- the founder, insert the attacker — post-image count still 1) cannot ride it.
            (old.Key is null and (select count(1) from committed.OwnerKey) = 0)

                -- or a pre-existing owner authorizes by signing over THIS row (Key, StampId); single-use via unique StampId

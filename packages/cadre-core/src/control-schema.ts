...
declare schema CadreControl {
    ...
    table Strand (
        ...
        constraint AuthorizedInsert check on insert (
            ...
            exists (select 1 from OwnerKey A where A.Key = context.OwnerKey and verify(digest('CadreControl.Strand', 'add', new.Id, new.Type, coalesce(new.MemberPrivateKey, ''), new.StampId), context.Signature, A.Key, 'ed25519'))
                ...
                --     that has ever been tombstoned. The usage-row clause is arguably subsumed by
                --     this one (a usage row implies the strand row existed, and its removal must
                --     file a tombstone), but it does not depend on RowKey being populated
                --     correctly; both are stated, per the style note above. The owner-signed branch
                --     is deliberately unaffected, so re-join stays owner-gated exactly as described.
                --     RowKey is owner-attested, not schema-verified (see Revocation.Authorized), so
                --     an owner can file a tombstone naming an id that never existed and thereby
                --     permanently foreclose consent-seating of that id — owner-only, the owner
                --     already controls invite issuance, and unbound ids are 128 random bytes minted
                --     at redemption, so no escalation (same reasoning as the pre-plant note above).
    ...
    table Revocation (
        ...
        -- RowKey is owner-ATTESTED, not schema-verified: positively binding it to the retired
        -- row (e.g. requiring a committed.Strand row carrying the pair) would reject a
        -- tombstone re-issued in a LATER transaction — which the delete-while-alone
        -- durability path (docs/architecture.md) plans to do — and reject it outright on a
        -- node that never converged on the removed row. Consequence: an owner can file a
        -- tombstone naming an id that never existed and thereby permanently foreclose
        -- CONSENT-seating of that id (Strand.AuthorizedInsert). Owner-only, the owner already
        -- controls invite issuance, and unbound ids are 128 random bytes minted at redemption
        -- — no escalation (same reasoning as the pre-plant note on RowIsGone).

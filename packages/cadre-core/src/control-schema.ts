...
declare schema CadreControl {
    ...
    table FormationUsage (
        Token text,
        UseNumber int,
        UsageStampId text not null unique,  -- single-use nonce for THIS redemption. The redeeming node mints it
                                        -- BEFORE it asks the approver, and it is bound into the 'vouch' digest
                                        -- below, so one sign-off is spendable exactly once.
                                        -- Unlike every other StampId in this schema, this table is append-only
                                        -- (InsertOnly below), so the nonce is never freed by a delete: \`unique\`
                                        -- alone makes it permanent and there is deliberately NO NotRevoked /
                                        -- RevocationRecorded pair here — do not go looking for the missing one.
                                        -- NOTE: \`unique\` is evaluated against LOCALLY VISIBLE rows, so two nodes
                                        ...
                                        -- nobody approved.
        PeerId text not null,           -- the joining peer this redemption is for, bound into the 'vouch' digest
                                        -- below so an approval cannot be re-filed under a different joiner's name.
                                        -- Writer-asserted: context.PeerSignature is declared but no constraint
                                        -- verifies it, so this records who the approval NAMES, not proof that the
                                        -- named peer consented — see
                                        -- tickets/backlog/debt-formation-usage-peer-signature-unverified.md.
        Disclosure text,
        StrandId text,
            ...
            exists (
                select 1 from FormationInvite FI
                    where FI.Token = new.Token
                        ...
                        -- (Token, UsageStampId, StrandId, PeerId, Disclosure), so an approval is NOT
                        ...
                        -- approval VERBATIM repeats its UsageStampId and is refused by that column's
                        -- \`unique\`; presenting it under any OTHER redemption changes a signed field and
                        -- fails this verify.
                        -- Why a fresh nonce rather than binding UseNumber: UseNumber is derived from the
                        -- table's current state (max+1, see Monotonic), so binding it would make the
                        -- approver sign a value a concurrent redemption can invalidate — the loser of the
                        -- race would need a SECOND approval for the same joiner (for a manual review
                        -- queue, a second human review). A nonce chosen freely by the redeeming node is
                        -- just as single-use and lets the loser retry the same approval under a new use
                        -- number.
                        ...
                        and (FI.ValidationUrl is null or exists (
                            select 1 from ValidationKey VK
                                where VK.Key = context.ValidationKey
                                    and verify(digest('CadreControl.FormationUsage', 'vouch', new.Token, new.UsageStampId, new.StrandId, new.PeerId, new.Disclosure), context.ValidationSignature, VK.Key, 'ed25519')))
    ...
    -- context.PeerSignature is declared but NO constraint reads it (the joining peer's signature is
    -- not carried over the formation protocol yet); the joiner itself is the PeerId COLUMN above, so
    -- the vouch digest is built from new.* only, like every other authorization rule here.

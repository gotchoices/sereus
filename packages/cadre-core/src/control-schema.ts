...
declare schema CadreControl {
    ...
    -- sub-collection as well as the table, and \`tickets/fix/strand-unique-index-sync-stale-revision\`
    -- is an open, intermittent engine failure in exactly that step (a table's rows commit while its
    -- index does not). Nothing has been observed here, and redemptions are rare writes; but if
    -- joins start failing intermittently with an index-sync error naming FormationUsage, this
    -- declaration is the first thing to suspect.

...
declare schema CadreControl {
    ...
    table Strand (
        ...
        constraint AuthorizedInsert check on insert (
            ...
            exists (select 1 from OwnerKey A where A.Key = context.OwnerKey and verify(digest('CadreControl.Strand', 'add', new.Id, new.Type, coalesce(new.MemberPrivateKey, ''), new.StampId), context.Signature, A.Key, 'ed25519'))
                ...
                --     RESIDUAL: the rule keys off a SURVIVING usage row, so it binds only ids that
                --     ever carried one. A strand seated purely owner-signed (insertStrand, no consent
                --     record) and then removed leaves nothing behind, so a spare unbound invite use
                --     can still seat that id afterwards — open and keyless, so no secret is chosen,
                --     but the removal does not stick. Tracked in
                --     tickets/backlog/bug-consent-reseats-owner-only-removed-strand-id.md.
                or (
                    new.Type = 'o'
    ...
    table Revocation (
        ...
        -- The digest binds the WHOLE row (TableName, StampId); the 'CadreControl.Revocation'

 ...
 *
 * Re-measure by reading the `[storage-op-budget]` lines this spec prints; they carry
 * the same per-method breakdown the blocked ticket tabulates. When a change moves a
 * number, update BOTH the measurement and its date here — a budget with no provenance
 * cannot tell the next reader whether the count grew or the budget was always wrong.
 */
 ...
 * not of the backend.
 */
const COLD = { ops: 1541, blocks: 21, opBudget: 1700, blockBudget: 24 };
/**
 ...
 */
const WARM = { ops: 315, blocks: 22, opBudget: 360, blockBudget: 25 };
...
function formatSnapshot(label: string, snapshot: OpSnapshot): string {
	const rows = snapshot.byMethod.map((m) => `${m.method} ${m.calls}/${m.blocks}`).join(', ');
	return `[storage-op-budget] ${label}: ${snapshot.total} ops over ${snapshot.distinctBlocks} distinct blocks — ${rows}`;
}
...
function expectWithinBudget(
	phase: string,
	snapshot: OpSnapshot,
	budget: { ops: number; blocks: number; opBudget: number; blockBudget: number }
): void {
	...
	expect(
		...
		+ 'See tickets/blocked/optimystic-block-read-amplification-on-control-start.md.'
	...
	expect(
		...
		+ 'or the amplification genuinely improved — in which case re-measure and TIGHTEN the budget rather than leaving the headroom.'

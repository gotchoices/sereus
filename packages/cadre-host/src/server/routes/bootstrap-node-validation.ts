/**
 * Shape validation for the requester-supplied `bootstrapNodes` of `POST /grants`
 * — the requester's control-network addresses the donated node is started with
 ...
 *
 * Requiring a multiaddr also closes a downstream ambiguity on the *other* side of
...
const log = debug('cadre:host:bootstrap-nodes');
 ...
 * provision request carries exactly what was validated.
 */
 ...
 * produces a donated node that either dies at boot or comes up permanently alone
 * — the requester learning about it, if ever, from donation status rather than
 * from a 400 naming the bad address.
 *

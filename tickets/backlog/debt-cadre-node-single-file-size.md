...
## Re-measured 2026-09-15 (review of `bug-strand-relay-reservation-not-resupervised`)

`wc -l` → **6828** `packages/cadre-core/src/cadre-node.ts`, **1194** `packages/cadre-core/src/strand-instance-manager.ts`. The node file is up from the 6496 recorded above on 2026-09-10; the strand relay-supervisor work added ~70 lines to it (`announceDelegateToRelay` and the `launchStrand` wiring) and ~100 to the manager (`startRelaySupervisors`, `awaitFirstRelayAttempts`, the `relaySupervisors` map and its `releaseRuntime` teardown). The manager is now more than double the 535 lines this ticket opened with, growing by the same mechanism: every per-strand background loop (backfill, membership reconciler, revocation enforcer, now relay supervisors) lands as another map on the same class plus a start site in `buildStrandRuntime` and a stop site in `releaseRuntime`. Evidence for the existing ticket, not a new one — if the split ever happens, a per-strand "runtime components" object with a uniform start/stop would take those four maps with it.

...
The upstream ticket named as the unblock condition has landed: optimystic `complete/a-reader-cannot-tell-its-view-stopped-advancing` (review `e66597f4`, 2026-09-06). A commit now counts as proof of freshness only when its approval quorum intersects a majority of the responsible cohort. Upstream could not run this repo's scenario to check it. `2-sync-fail-fast-on-a-stalled-revision-view` (`09ed71bb`) also landed the same day.

**The symptom has persisted since then, at a lower rate:** 2026-09-10 on 1.0.0-beta.2 (one of 4 edge runs; the other 3 were the since-fixed Revocation read), and 2026-09-16 (1 of 9 edge runs, and the ninth run after `control-trio-b-connects-to-c-before-sever`). No trace was taken on either date, so it is unknown whether the upstream fix works and something else remains, or whether it does not reach this path.

To verify: run `control-cohort-edge-carries-data` and `control-cohort-three-node-isolation` in isolation until one fails, with `DEBUG='optimystic:db-p2p:*,optimystic:db-core:collection'` (use the trailing star, because channel names carry a peer-id suffix). Check whether B's view of C's `CadrePeer` row still stops at the owner-vouch revision. If it does, file the new trace upstream as a follow-up to that ticket. If it does not, re-diagnose from the new trace.

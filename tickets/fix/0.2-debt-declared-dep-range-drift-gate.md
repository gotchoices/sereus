...
once. On 2026-07-29 it drifted again: `../optimystic` released **0.17.0** while all 22
`@optimystic/*` ranges in this repo still declared `^0.16.3`, which for a `0.x` version excludes
0.17.0 entirely. The gap this time spanned four `db-p2p` fixes to a defect chain that made two-node
...
- It must cover **every** workspace package, not just those the integration suite exercises. The
  drift included the `reference-app-*` packages, which the integration suite never touches but a
  consumer installs. (`cadre-host` and `cadre-provider` were named in the original report but in
  fact declare no `@optimystic/*` or `@quereus/quereus` dependency at all — verified 2026-07-29.
  The gate should still walk them, so a future direct dependency is covered from day one.)

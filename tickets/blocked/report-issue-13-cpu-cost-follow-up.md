...
>
> **Supplying native crypto.** Yes in principle. It has to go through two layers: an option on optimystic's `createLibp2pNode`, then passed through by cadre-core's node options. We'll confirm the exact shape on the optimystic side before you start, so the PR only needs doing once.
>

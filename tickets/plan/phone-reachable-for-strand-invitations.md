...
- **Strand networks too.** Formation needs the host's strand nodes reachable, and strand nodes reserve relay slots only through the `'configured'` route from `relayAddrs` (`relay-addrs.ts` module doc). Whatever supplies the control node's relay must reach strand instances as well. Note the open fix ticket `bug-strand-relay-reservation-not-resupervised` on the same mechanism.

...
- `sereus-node/`: **Pointer only** (see `sereus-node/README.md`) — the headless cadre-node Docker template lives at `../../packages/cadre-cli/docker/` (canonical, ships with `@serfab/cadre-cli`). Not a shared ops service like the ones below, so it has no `env.example`/`docker-compose.yml` here and isn't wired into `./sereus/ops/scripts/install`.
